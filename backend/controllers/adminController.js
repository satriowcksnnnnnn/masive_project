...
const postSeniman = async (req, res) => {
    ...
    try {
        const [result] = await database.query(`INSERT INTO seniman (nama, deskripsi, foto) VALUES ('${nama}', '${deskripsi}', '${foto}')`);
...
const getSenimanById = async (req, res) => {
    ...
    try {
        const [results] = await database.query(`SELECT * FROM seniman WHERE id = '${id}'`)
...
const getJenisKarya = async (req, res) => {
    try {
        const [result] = await database.query(`SELECT * FROM jenis_karya`);
    } catch (error) {
        console.log(error);
        res.status(500).json({
...
const postJenisKarya = async (req, res) => {
    ...
    try {
        const [result] = await database.query(`INSERT INTO jenis_karya (jenis) VALUES ('${jenis}')`);
...
const getJenisKaryaById = async (req, res) => {
    ...
    try {
        const [results] = await database.query(`SELECT * FROM jenis_karya WHERE id = '${id}'`)
        if (Array.isArray(results) && results.length < 0) {
            res.json({
                jenis_karya: [],
            })
...
const getBeritaById = async (req, res) => {
    ...
    try {
        const [results] = await database.query(`SELECT * FROM berita WHERE id = '${id}'`)
        if (Array.isArray(results) && results.length < 0) {
            res.json({
                berita: [],
            })
...
const postBerita = async (req, res) => {
    ...
    try {
        const [result] = await database.query(`INSERT INTO berita (judul, img, deskripsi) VALUES ('${judul}', '${img}', '${deskripsi}')`);
...
const getAgendaById = async (req, res) => {
    ...
    try {
        const [results] = await database.query(`SELECT * FROM agenda WHERE id = '${id}'`)
        if (Array.isArray(results) && results.length < 0) {
            res.json({
                agenda: [],
            })
...
const postAgenda = async (req, res) => {
    const { judul, desc_singkat, penyelenggara, lokasi, tanggal, waktu } = req.body;

    try {
        const [result] = await database.query(`INSERT INTO agenda (judul, desc_singkat, penyelenggara, lokasi, tanggal, waktu) VALUES ('${judul}', '${desc_singkat}', '${penyelenggara}', '${lokasi}', '${tanggal}', '${waktu}')`);
...
const deleteAgendaById = async (req, res) => {
    ...
    try {
        const [result] = await database.query(`DELETE FROM agenda WHERE id = '${id}'`)
...
const getAllKarya = async (req, res) => {
    ...
    try {
        const [results] = await database.query(`SELECT * FROM karya`);
        if (Array.isArray(results) && results.length < 0) {
            res.json({
                karya: [],
            });
...
const updateKaryaById = async (req, res) => {
    const { id } = req.params;
    const { id_user, nama, email, id_jenis, alamat, judul_karya, deskripsi, status, threeD } = req.body;
...
const getDashboardCounts = async (req, res) => {
    try {
        const [senimanCount] = await database.query(`SELECT COUNT(*) AS count FROM seniman`);
        const [beritaCount] = await database.query(`SELECT COUNT(*) AS count FROM berita`);
        const [karyaCount] = await database.query(`SELECT COUNT(*) AS count FROM karya`);
        const [agendaCount] = await database.query(`SELECT COUNT(*) AS count FROM agenda`);
